import pytest
from account.views import account_view, login_view, logout_view, registration_view
...
def test_login_route(client):
    ...
    response = client.post(
        reverse("login"), {"email": "testuser@gmail.com", "password": "UnMotDePasse12"}
    )
...
def test_login_route_failed(client):
    ...
    """
    response = client.post(
        reverse("login"), {"email": "Notagoodone", "password": "Notagoodone"}
    )
    assertTemplateUsed(response, "account/login.html")
...
def test_register_route(client):
    ...
    """
    Test approach starts with testing if the 'registration'
    route maps to 'registration_view'. Then we test
    if the registration_view renders the correct template ( account/register.html )
    with correct Form ( RegisterUserForm ).
    After that we create a temporary user, by using our 'register' route
