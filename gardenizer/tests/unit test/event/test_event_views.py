...
def test_event_add_maintenance_view_form_valid(
    client, authenticated_user, yes_add_maintenance_event_form_data, create_categories
):
    """test valid post data on add maintenance event view"""
    response = client.post(
        "/add-maintenance-event", data=yes_add_maintenance_event_form_data
    )
...
def test_event_add_maintenance_view_form_not_valid(
    client, authenticated_user, create_categories, no_add_maintenance_event_form_data
):
    """test invalid post data on add maintenance event view"""
    response = client.post(
        "/add-maintenance-event", data=no_add_maintenance_event_form_data
    )
...
def test_get_update_customer_event_view(
    client, authenticated_user, create_maintenance_event
):
    """test the get method for the update customer event view"""
    event = Evenement.objects.all()
    response = client.get(
        reverse("update_customer_event", kwargs={"eventid": event[0].id})
    )
    assert response.status_code == 200
...
def test_post_update_customer_event_view(
    ...
    response = client.post(
        reverse("update_customer_event", kwargs={"eventid": event[0].id}), data=payload
    )
    assert response.status_code == 302
...
def test_get_method_update_maintenance_event_view(
    client, authenticated_user, create_customer_event
):
    """test the get method for update maintenance event view"""
    event = Evenement.objects.all()
    response = client.get(
        reverse("update_maintenance_event", kwargs={"eventid": event[0].id})
    )
    assert response.status_code == 200
    assertTemplateUsed("event/edit_event.html")
...
@pytest.mark.django_db
def test_post_customer_event_view(client, authenticated_user, create_maintenance_event):
...
@pytest.mark.django_db
def test_delete_event_view(client, authenticated_user, create_maintenance_event):
...
@pytest.mark.django_db
def test_post_delete_event_view(client, authenticated_user, create_maintenance_event):
...
def test_valid_post_method_add_customer_view(
    client, authenticated_user, yes_add_customer_form_data
):
...
def test_invalid_post_method_add_customer_view(
    client, authenticated_user, no_add_customer_form_data
):
...
@pytest.mark.django_db
def test_get_delete_customer_view(client, authenticated_user, create_customer):
    """test the get method of the delete customer view"""
    customer = Customer.objects.all()
    response = client.get(
        reverse("customer_delete", kwargs={"customerid": customer[0].id})
    )
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_post_delete_customer_view(client, authenticated_user, create_customer):
...
def test_valid_post_edit_customer_view(
    client, authenticated_user, create_customer, create_city
):
    ...
    response = client.post(
        reverse("customer_update", kwargs={"customerid": customer[0].id}), data=payload
    )
    assert response.status_code == 302
