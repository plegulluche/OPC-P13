...
class Calendar(HTMLCalendar):
    ...
    def formatday(self, day, events):
        ...
        if day != 0:
            return (f'<td><span class="date">'
                    f'<a class="cal-link" href="day/{self.month}/{day}">{day}</a>'
                    f'</span><ul> {d} </ul></td>')
        return "<td></td>"
    ...
    def formatmonth(self, userid, withyear=True):
        ...
        events = Evenement.objects.filter(
            event_start__year=self.year, event_start__month=self.month, user=user
        )

        cal = '<table border="0" cellpadding="0" cellspacing="0" class="calendar" style="color:white;">\n'
